...
export function handleSwap(event: Swap): void {
  ...
  pancake.untrackedVolumeUSD =
    pancake.untrackedVolumeUSD.plus(derivedAmountUSD);
  pancake.totalTransactions = pancake.totalTransactions.plus(ONE_BI);
  ...
  let pancakeDayData = updateKyotoDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  pancakeDayData.dailyVolumeUSD =
    pancakeDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  pancakeDayData.dailyVolumeBNB =
    pancakeDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  pancakeDayData.dailyVolumeUntracked =
    pancakeDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  pancakeDayData.save();
